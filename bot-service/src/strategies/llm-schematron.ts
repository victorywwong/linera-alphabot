...
/**
 * Schematron LLM Strategy for Wave 2+
 *
 * Uses Schematron models via inference.net (Atoma's production inference network)
 * Models: Schematron 3B (reasoning), Schematron 8B (structured output)
 * Cost: $0.02/$0.05 per million tokens (very affordable)
 * Migration path: inference.net → Atoma Network (same API format)
 */
export class SchematronStrategy implements Strategy {
  readonly name = 'schematron';
  ...
  constructor() {
    this.apiKey = process.env.SCHEMATRON_API_KEY || '';
    this.baseUrl = process.env.SCHEMATRON_BASE_URL || 'https://api.inference.net';
    this.model = process.env.SCHEMATRON_MODEL || 'schematron-3b'; // 3B for reasoning

    if (!this.apiKey) {
      throw new Error(
        'SCHEMATRON_API_KEY is required. Get one from https://inference.net'
      );
    }
  }
...
IMPORTANT: After your analysis, you MUST provide your final answer in the exact format specified.`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Schematron API error (${response.status}): ${errorText}`);
      }
      ...
      if (!result.choices || result.choices.length === 0) {
        throw new Error('Schematron API returned no choices');
      }

      const content = result.choices[0].message?.content;
      if (!content) {
        throw new Error('Schematron API returned empty content');
      }

      return this.parseResponse(content, data);
    } catch (error) {
      console.error('[SchematronStrategy] Prediction error:', error);
